...
6. Public `/verify/:jobId` is a forensic receipt. The page also `eth_call`s the receipt registry from the browser (`evmrpc.0g.ai`). API fields alone are not a pass. Jobs results include a **View proof** button. When the Router returns `x_0g_trace`, `/verify` shows the reported request id / provider / `tee_verified` and a locally computed keccak fingerprint — that is Router-reported evidence, not independent proof (EIP-191 recover vs the TEE signer is).
7. Address/tx inspect is live Aristotle RPC only (inline in Flow). Paid TeeML explanation is a separate quoted job. Beacon does not invent ABIs. Bridges are catalogued from Zia docs / Hub / get.0g.ai. Beacon Safe **cannot** sign the source chain. Flow can send the unsigned LI.FI request from the **user wallet** and poll until LI.FI is DONE **with a destination tx**. MCP `track_bridge` does the same poll — never complete from the source hash alone.
