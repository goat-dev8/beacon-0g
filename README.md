...
8. MCP: connect at `/flow/mcp`. One wallet signature mints a scoped grant (default 5 0G / tx, 20 0G / day). Endpoint `POST /mcp` with `Authorization: Bearer`. The agent never receives a private key. ERC-8004 `register()` is live (agent 3531902). `giveFeedback` uses the official 8-argument ABI on the Reputation **implementation** (the 130-byte address is a proxy). Feedback is posted only after a real job release or refund, from a client that is **not** the agent owner.
