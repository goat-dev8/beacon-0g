...
6. Public `/verify/:jobId` is a forensic receipt. The page also `eth_call`s the receipt registry from the browser (`evmrpc.0g.ai`). API fields alone are not a pass. Each closed job binds an **action hash** (`keccak256(abi.encode)` of request, policy, TEE, storage, lock, settle, receipt). Many action hashes can share one on-chain Merkle batch (`BeaconEvidenceAnchor`). Jobs results include a **View proof** button. When the Router returns `x_0g_trace`, `/verify` shows the reported request id / provider / `tee_verified` and a locally computed keccak fingerprint — that is Router-reported evidence, not independent proof (EIP-191 recover vs the TEE signer is).
7. Address/tx inspect is live Aristotle RPC only (inline in Flow). Paid TeeML explanation is a separate quoted job. Beacon does not invent ABIs. Ask “what did I do last week?” and Flow answers from History + Storage roots + on-chain receipts — not from an invented chat log.
8. Bridges: live LI.FI quotes in the **requested direction**. Proven executable quotes: native **0G → USDC on Base**, and **USDC on Base or Ethereum → USDC.e on 0G**. “Bridge 0.3 0G to USDC on Base” is not rewritten as Base → 0G. Unsupported chains (Solana, and others not in that set) return `UNSUPPORTED_ROUTE` and do not get a reverse quote. Beacon Safe **cannot** sign the source chain. Flow can send the unsigned LI.FI request from the **user wallet** and poll until LI.FI is DONE **with a destination tx for this same source hash**. MCP `track_bridge` does the same poll.
9. MCP: connect at `/flow/mcp`. One wallet signature mints a scoped grant (default 5 0G / tx, 20 0G / day). `POST /mcp` accepts `Authorization: Bearer`. Connect-Agent tokens last until the grant expires (default 7 days) so Cursor `mcp.json` does not die after one hour. Clients that speak OAuth can **Authenticate** via PKCE (`GET /.well-known/oauth-protected-resource`); those access tokens remain short-lived and refresh. Unauthenticated `GET /mcp` returns 401 + `WWW-Authenticate` — it is not an open tool stream. Every write still passes grant scope, TTL, cap, Safe policy, and deterministic preflight **plus eth_call simulation**. Spend tools report four ledgers (escrow, Safe window, Zia slice, gas) for 1d / 7d / 30d — never added together; on-chain `windowSpent` is the live rolling window and is shown under Today only. The agent never receives a private key. ERC-8004 `register()` is live (agent 3531902). `giveFeedback` uses the official 8-argument ABI on the Reputation **implementation** (the 130-byte address is a proxy). Feedback is posted only after a real job release or refund, from a client that is **not** the agent owner.
...
Pinned contracts live in `packages/shared/src/constants.ts` (W0G, Zia, Flow, Ledger, Inference, ERC-8004). Beacon contracts: escrow `0xFB9c10423EAaD015dDb04f5aC85273f1B3F7A566`, receipt registry `0x31666B7ECf736c0c6014F0cd63C646B7f4Af3887`, Safe factory `0x531e8533aBA2Ca534959Df860C3226c02EaC3eE1`, demo Safe `0x6A3388D833C09a00DDbbD4e1a6c11C9623717A30`.
...
- Beacon quotes LI.FI only for the directions above. It does not claim W0G outbound, 0G USDC.e → Base, or any route it has not executed a live quote for.
