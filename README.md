...
7. Address/tx inspect is live Aristotle RPC only (inline in Flow). Paid TeeML explanation is a separate quoted job. Beacon does not invent ABIs. Ask “what did I do last week?” and Flow answers from History + Storage roots + on-chain receipts — not from an invented chat log. Bridges are catalogued from Zia docs / Hub / get.0g.ai. Beacon Safe **cannot** sign the source chain. Flow can send the unsigned LI.FI request from the **user wallet** and poll until LI.FI is DONE **with a destination tx**. MCP `track_bridge` does the same poll — never complete from the source hash alone.
