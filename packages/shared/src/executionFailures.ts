...
export function looksLikeWalletError(raw: string): boolean {
  const text = raw.toLowerCase();
  return (
    /wallet error|error 4001|code 4001|user rejected the request|unrecognized chain|wrong network|nonce too low|already pending/.test(
      text,
    ) && !/\bswap\b|\bbridge\b|\binspect\b|\banalyze\b/.test(text)
  );
}
